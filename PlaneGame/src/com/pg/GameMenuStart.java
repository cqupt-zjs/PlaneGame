...
public class GameMenuStart {
	...
	//菜单初始化
	public GameMenuStart(Bitmap bmpMenu, Bitmap bmpButton, Bitmap bmpButtonPress) {
	...
	public void draw(Canvas canvas, Paint paint) {
		//绘制菜单背景图
		canvas.save();
		float sx=(float)(MySurfaceView.screenW)/bmpMenu.getWidth();
		float sy =(float)(MySurfaceView.screenH)/bmpMenu.getHeight();
		canvas.scale(sx, sy);
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE) {
			...
			if (pointX > btnX && pointX < btnX + bmpButton.getWidth()) {
				if (pointY > btnY && pointY < btnY + bmpButton.getHeight()) {
					...
					MySurfaceView.gameGoon=false;
					MySurfaceView.newGame=true;
				}
			}
		}
	}
